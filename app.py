    ...
    if os.path.exists(USERS_FILE):
        try:
            with open(USERS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            return data if isinstance(data, dict) else {"admin": hash_password("123")}
        except:
            return {"admin": hash_password("123")}
    else:
        return {"admin": hash_password("123")}
...
def save_last_login(username, remember):
    try:
...
def init_user_db():
    ...
    if 'roles_db' not in st.session_state:
        st.session_state['roles_db'] = load_roles()
    if 'jobs_db' not in st.session_state:
        st.session_state['jobs_db'] = load_jobs()
    if 'applications_db' not in st.session_state:
        st.session_state['applications_db'] = load_applications()
    ...
    with col2:
            ...
            with st.form("login_form"):
                ...
                if submit_login:
                    if not username or not password:
                        ...
                        stored = st.session_state['users_db'][username]
                        if stored == hash_password(password) or stored == password:
                            st.session_state['authenticated'] = True
                            st.session_state['username'] = username
                            
                            # Load Role
                            role = st.session_state['roles_db'].get(username, "Student")
                            st.session_state['role'] = role
                            
                            save_last_login(username, remember_me)
        ...
        with tab_signup:
            with st.form("signup_form"):
                ...
                new_role = st.selectbox("SELECT ROLE", ["Student", "Employer"])
                ...
                if submit_signup:
                    if not new_user or not new_pass:
                        ...
                        st.session_state['roles_db'][new_user] = new_role
                        save_users(st.session_state['users_db'])
                        save_roles(st.session_state['roles_db'])
...
def Hire_IQ_app():
    ...
    def employer_post_job(username):
        ...
        with st.form("post_job_form"):
            ...
            with col2:
                ...
                url = st.text_input("Application URL (optional)", value="#")
            ...
            if submitted:
                if not title or not company:
                    ...
                    new_job = {
                        ...
                        "skills_match": [s.strip() for s in skills.split(',')] if skills else [],
                        "url": url,
                        "salary": salary,
                        ...
                        "posted_by": username,
                        "match_percentage": 0 # Default
                    }
                    st.session_state['jobs_db'].append(new_job)
                    save_jobs(st.session_state['jobs_db'])
                    st.success("Job posted successfully!")

    def employer_view_applicants(username):
        st.header("Applicants")
        # Filter jobs posted by this employer
        my_jobs = [j for j in st.session_state.get('jobs_db', []) if j.get('posted_by') == username]
        
        if not my_jobs:
            st.info("You haven't posted any jobs yet.")
            return

        job_titles = [j['title'] for j in my_jobs]
        selected_job = st.selectbox("Filter by Job", ["All"] + job_titles)
        
        apps = st.session_state.get('applications_db', [])
        
        # Filter apps
        if selected_job != "All":
            # Find job id
            job_id = next((j['id'] for j in my_jobs if j['title'] == selected_job), None)
            display_apps = [a for a in apps if a['job_id'] == job_id]
        else:
            # All apps for my jobs
            my_job_ids = [j['id'] for j in my_jobs]
            display_apps = [a for a in apps if a['job_id'] in my_job_ids]
            
        if not display_apps:
            st.info("No applicants found.")
        else:
            for app in display_apps:
                with st.expander(f"{app['applicant_name']} - {app['job_title']} ({app['status']})"):
                     st.write(f"**Email:** {app.get('applicant_email', 'N/A')}")
                     st.write(f"**Date Applied:** {app.get('date_applied', 'N/A')}")
                     
                     current_status = app.get('status', 'Applied')
                     status_options = ["Applied", "Reviewing", "Interview", "Hired", "Rejected"]
                     try:
                        index = status_options.index(current_status)
                     except:
                        index = 0
                        
                     new_status = st.selectbox("Update Status", status_options, key=f"status_{app['id']}", index=index)
                     if new_status != current_status:
                         app['status'] = new_status
                         save_applications(st.session_state['applications_db'])
    ...
    def search_jobs(skills, location="", num_results=5, salary_range=None, date_filter="Any time", company_filter="Any size", job_type_filter="All Types", experience_level_filter="All Levels"):
        # This is a mock function that would normally call a job API like Adzuna
        # For demonstration purposes, we'll return mock data
        # Load jobs from database
        mock_jobs = st.session_state.get('jobs_db', [])
        ...
        if location:
            ...
            filtered_jobs = [job for job in filtered_jobs if loc in job["location"].lower()]
            if not filtered_jobs and loc not in ("", "any"):
                fallback_notes.append("No jobs in selected location; expanded to all locations.")
                filtered_jobs = mock_jobs  # relax location if it eliminated everything
        ...
        if salary_range:
            ...
            if not filtered_jobs:
                fallback_notes.append("No jobs in selected salary range; widened range.")
                filtered_jobs = mock_jobs  # relax salary if it eliminated everything
        ...
        if date_filter != "Any time":
            ...
            if not filtered_jobs:
                fallback_notes.append("No jobs with selected date; included older postings.")
                filtered_jobs = mock_jobs  # relax date if it eliminated everything
        ...
        if company_filter != "Any size":
            ...
            if not filtered_jobs:
                fallback_notes.append("No jobs at selected company size; expanded to any size.")
                filtered_jobs = mock_jobs  # relax company size if it eliminated everything
        
        # Filter by job type
        if job_type_filter and job_type_filter != "All Types":
            jt = job_type_filter.lower()
            # broader match: match job_type OR title/description hints
            def jobtype_match(job):
                jt_text = job.get('job_type', '').lower()
                title = job.get('title', '').lower()
                desc = job.get('description', '').lower()
                if jt in jt_text:
                    return True
                if jt == "internship":
                    return ("intern" in title) or ("intern" in desc)
                if jt == "contract":
                    return ("contract" in jt_text) or ("contract" in title) or ("contract" in desc)
                return False
            filtered_jobs = [job for job in filtered_jobs if jobtype_match(job)]
            if not filtered_jobs:
                fallback_notes.append("No jobs for selected job type; showing all job types.")
                filtered_jobs = mock_jobs  # relax job type if it eliminated everything
            
        # Filter by experience level
        if experience_level_filter and experience_level_filter != "All Levels":
            exp = experience_level_filter.lower()
            filtered_jobs = [job for job in filtered_jobs if exp in job.get('experience_level', '').lower()]
            if not filtered_jobs:
                fallback_notes.append("No jobs for selected experience; expanded to all levels.")
                filtered_jobs = mock_jobs  # relax experience if it eliminated everything
        ...
        # Record any fallback notes to session for UI
        if fallback_notes:
            st.session_state['search_note'] = " ".join(fallback_notes)
        else:
    ...
    if role == 'Employer':
        tabs = st.tabs(["📝 Post a Job", "👥 Applicants", "📋 My Jobs"])
        
        with tabs[0]:
            employer_post_job(username)
            
        with tabs[1]:
            employer_view_applicants(username)
            
        with tabs[2]:
            st.header("My Posted Jobs")
            my_jobs = [j for j in st.session_state.get('jobs_db', []) if j.get('posted_by') == username]
            if my_jobs:
                # Custom table or simple display
                 for job in my_jobs:
                     with st.expander(f"{job['title']} ({job['date_posted']})"):
                         st.write(job)
            else:
                st.info("No jobs posted yet.")
        
        return # STOP here for Employer

    # Student View
    tabs = st.tabs(["📄 Resume Analyzer", "🔍 Job Search", "🧭 Career Advisor", "📊 Dashboard & Insights", "📂 My Applications"])
        ...
        with col_charts_1:
            ...
            if jobs_count > 0:
                ...
                if len(unique_types) < 2:
                    labels = ['Full-time', 'Remote', 'Contract', 'Internship']
                    values = [jobs_count, max(1, jobs_count//2), max(1, jobs_count//3), max(1, jobs_count//4)]
                else:
                    # जर डेटा खरा आणि वेगळा असेल तर तोच वापरा
                    from collections import Counter
                    counter = Counter(job_types)
            ...
            # Build pie chart without pandas
            import plotly.graph_objects as go
            fig_pie = go.Figure(data=[go.Pie(
                labels=labels,
                values=values,
                hole=0.5,
                marker=dict(colors=[color_map.get(lbl, '#AB47BC') for lbl in labels])
            )])
        ...
        if st.session_state.extracted_skills:
            ...
            if st.button("Find Jobs"):
                with st.spinner("Searching for jobs that match your skills..."):
                    ...
                    jobs = search_jobs(
                        ...
                        experience_level_filter=experience_filter
                    )
            ...
            if st.session_state.job_recommendations:
                if len(st.session_state.job_recommendations) > 0:
                    ...
                    if st.session_state.get('search_note'):
                        ...
                        for i, job in enumerate(display_jobs):
                            ...
                            with st.expander(expander_header, expanded=False):
                                ...
                                with col_right:
                                    ...
                                    col1, col2 = st.columns(2)
                                    with col1:
                                        if st.button(f"Apply Now", key=f"apply_job_{i}"):
                                            # Check if already applied
                                            existing_apps = st.session_state.get('applications_db', [])
                                            current_job_id = job.get('id', f"{job['title']}_{job['company']}")
                                            
                                            already_applied = any(a['job_id'] == current_job_id and a['applicant_name'] == username for a in existing_apps)
                                            ...
                                            if already_applied:
                                                ...
                                                st.session_state['applications_db'].append(new_app)
                                                save_applications(st.session_state['applications_db'])
                                                st.success("Applied!")
                                    with col2:
                            ...
                            with st.expander(f"Your Saved Jobs ({len(st.session_state.saved_jobs)})", expanded=False):
                                ...
                                with col1:
                                    if st.button("Download Saved Jobs"):
                                        ...
                                        # Build CSV content without pandas
                                        output = io.StringIO()
                                        if saved_jobs_data:
                                            fieldnames = list(saved_jobs_data[0].keys())
                                        else:
                                            fieldnames = ["Title","Company","Location","Salary","Experience Level","Match Percentage","URL","Date Posted","Matching Skills","Required Skills"]
                                        writer = csv.DictWriter(output, fieldnames=fieldnames)
                                        writer.writeheader()
                                        for row in saved_jobs_data:
                                            writer.writerow(row)
...
**Result:** Share the outcomes and what you learned"""
                    ...
                    st.warning("No jobs found matching your criteria. Expanding search to show best matches.")
                    # Attempt a fallback search with relaxed filters
                    jobs = search_jobs(
                        skills=st.session_state.last_search['skills'] if 'last_search' in st.session_state else st.session_state.extracted_skills,
                        location="",
                        num_results=st.session_state.get('last_search', {}).get('num_results', 5) if 'last_search' in st.session_state else 5,
                        salary_range=None,
                        date_filter="Any time",
                        company_filter="Any size",
                        job_type_filter="All Types",
                        experience_level_filter="All Levels"
                    )
                    st.session_state.job_recommendations = jobs
                    if jobs:
                        st.success(f"Showing {len(jobs)} best-matching jobs after relaxing filters.")
                    else:
                        st.info("No mock jobs available for your selection. Try clearing filters or adding more skills.")
        else:
    ...
    with tabs[4]:
        ...
        my_apps = [a for a in st.session_state.get('applications_db', []) if a.get('applicant_name') == username]
